    ...
    function () {
      ...
      */
      ...
      Wfst.prototype._addControlTools = function () {
        ...
        var html = Object.keys(this._mapLayers).map(function (key) {
                ...
                return __generator(this, function (_b) {
                  switch (_b.label) {
                    case 0:
                      ...
                      payload = this._xs.serializeToString(transaction); // Ugly fix to support GeometryCollection on GML
                      // See https://github.com/openlayers/openlayers/issues/4220
                      ...
                      if (this._geoServerData[layerName].geomType === 'GeometryCollection') {
                        if (mode === 'insert') {
                          ...
                          payload = payload.replace(/(.*)<Name>geometry<\/Name>(<Value>)(.*?)(<\/Value>)(.*)/g, "$1<Name>" + this._geoServerData[layerName].geomField + "</Name>$2" + gmemberIn + "$3" + gmemberOut + "$4$5"); // payload = payload.replaceAll(`<Name>geometry</Name><Value>`, `<Name>geometry</Name><Value><MultiGeometry xmlns="http://www.opengis.net/gml" srsName="${srs}"><geometryMember>`);
